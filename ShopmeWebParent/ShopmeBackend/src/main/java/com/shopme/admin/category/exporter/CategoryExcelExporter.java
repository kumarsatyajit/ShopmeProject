...
public class CategoryExcelExporter extends AbstractExporter {
	...
	private void writeHeaderLine() {
		this.sheet = workbook.createSheet("category_data");
		...
		createCell(row, 3, "ImageUrl", cellStyle);
		createCell(row, 4, "Enabled", cellStyle);
		createCell(row, 5, "Parent", cellStyle);
	}
	...
	private void writeDataLines(List<Category> categories) {
		...
		for (Category category : categories) {
			...
			createCell(row, columnIndex++, category.getCategoryImage(), cellStyle);
			createCell(row, columnIndex++, category.isEnabled(), cellStyle);
			createCell(row, columnIndex, category.getParentName(), cellStyle);
		}
	}
